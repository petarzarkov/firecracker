...
/** The audit trail, written by SQLite itself. */
export class AuditTriggers {
  /** `banned` is stored as 0/1; json_object would emit a number, so coerce it. */
  static #snapshot(alias: string, columns: readonly string[]): string {
   ...
   */
  static apply(sqlite: BunSqlite): void {
  ...
  `);
    ...
    for (const { table, entity, columns } of AUDITED_TABLES) {
      ...
      BEGIN
        ...
        VALUES (${UUID_EXPRESSION}, ${actor}, 'INSERT', '${entity}', NEW.id, NULL, ${AuditTriggers.#snapshot('NEW', columns)}, unixepoch('subsec') * 1000);
      END
    `);
      ...
      BEGIN
        INSERT INTO audit_log (id, actor_id, action, entity_name, entity_id, old_values, new_values, created_at)
        VALUES (${UUID_EXPRESSION}, ${actor}, 'UPDATE', '${entity}', NEW.id, ${AuditTriggers.#snapshot('OLD', columns)}, ${AuditTriggers.#snapshot('NEW', columns)}, unixepoch('subsec') * 1000);
      END
    `);
      ...
      BEGIN
        INSERT INTO audit_log (id, actor_id, action, entity_name, entity_id, old_values, new_values, created_at)
        VALUES (${UUID_EXPRESSION}, ${actor}, 'DELETE', '${entity}', OLD.id, ${AuditTriggers.#snapshot('OLD', columns)}, NULL, unixepoch('subsec') * 1000);
      END
    `);
    }
  }

  /** Stamps who the next write in this connection belongs to. */
  static setActor(sqlite: BunSqlite, actorId: string | null): void {
