...
/**
 * Everything `betterAuth()` takes that only exists inside the container: the
 * database handle `DatabaseModule` opened, the session store, and the queue a
 * password reset goes onto.
 *
 * Named rather than inline in the decorator: the return is annotated, because
 * nothing else gives `sendResetPassword`'s `{ user, url }` a contextual type and
 * it infers as `any`.
 */
...
@Module({
  ...
  imports: [
    AccountLinkerModule,
    // The mount is the second, **synchronous** argument: the route table is built
