...
/**
 * The arithmetic of the game, in one place and with no container behind it, so it
 * can be unit-tested and read on its own.
 *
 * Everything here works in **integer hundredths** of a multiplier. `1.07x` is
 * `107`. See `game-round.schema.ts` for why the storage layer does the same.
 */

/** A multiplier as clients see it: `107` becomes `1.07`. */
export const toMultiplier = (x100: number): number => x100 / 100;
 ...
 * paid.
 */
export const multiplierAtX100 = (elapsedMs: number, divisor: number): number =>
  Math.floor(Math.exp(elapsedMs / divisor) * 100);
 ...
 */
export const payoutCents = (
  betAmountCents: number,
  multiplierX100: number,
): number => Math.floor((betAmountCents * multiplierX100) / 100);
 ...
 */
export const DEFAULT_RNG_ALGORITHM: RngAlgorithm = 'pcg64';

/** Probability of an instant 1.00x crash. This is the house edge. */
const INSTANT_CRASH_PROBABILITY = 0.03;
 ...
 */
export const fairnessSeed = (
  serverSeed: string,
  clientSeed: string,
  nonce: number,
): string => `${serverSeed}:${clientSeed}:${nonce}`;

/**
 * The crash point, in hundredths, drawn deterministically from the three published
 * values.
 *
 * ## Why a seeded PRNG rather than a hash
 *
 * The Postgres version ran HMAC-SHA256 and then did
 * `parseInt(hash.slice(0, 13), 16)` - 52 bits carved out of a 256-bit digest with
 * the remaining 204 thrown away, and the `h % 33 === 0` house-edge test riding on
 * the low bits of that slice. It worked, but the uniformity was incidental.
 *
 * `@arkv/rng` seeds deterministically from a string (FNV-1a to u64, in Rust) and
 * draws with unbiased rejection sampling, so `float()` is uniform in [0, 1) by
 * construction rather than by accident.
 *
 * **This is still provably fair.** Everything the draw consumes - server seed,
 * combined client seed, nonce, algorithm - is published once the round crashes, so
 * a player reproduces this exact number. What changed is the tool they verify with:
 * `@arkv/rng` at the round's recorded algorithm, rather than three lines of SHA256.
 *
 * The distribution is unchanged: ~3% instant crash, and `P(crash >= x) ~ 0.99 / x`
 * above that.
 */
export const crashPointX100 = (
  serverSeed: string,
  clientSeed: string,
  nonce: number,
  algorithm: RngAlgorithm = DEFAULT_RNG_ALGORITHM,
): number => {
  const rng = new Rng(fairnessSeed(serverSeed, clientSeed, nonce), algorithm);
  try {
    ...
    const u = rng.float();
    if (u < INSTANT_CRASH_PROBABILITY) return 100;
    ...
    return Math.max(100, Math.floor(HOUSE_EDGE_NUMERATOR / (1 - u)));
  } finally {
